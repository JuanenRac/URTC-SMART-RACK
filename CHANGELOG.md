...
- Still out of scope, on purpose: wiring `protocol.c`/`rack_command.c`/`link_watchdog.c` into a real UART/CAN receive path in `main.c` - there is still no PCB/transceiver to receive real frames from (see `mejoras_futuras.txt`).
